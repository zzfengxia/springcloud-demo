...
public class GatewayFlowRuleNacosController {
    ...
    @Autowired
    @Qualifier("gatewayFlowNacosProvider")
    private DynamicRuleProvider<RuleEntityWrapper<GatewayFlowRuleEntity>> gatewayFlowNacosProvider;
    @Autowired
    @Qualifier("gatewayFlowNacosPublisher")
    ...
    public Result<List<GatewayFlowRuleEntity>> queryFlowRules(String app) {
        ...
        try {
            ...
            if (rules != null && !rules.getRuleEntity().isEmpty()) {
                for (GatewayFlowRuleEntity entity : rules.getRuleEntity()) {
                    entity.setApp(app);
                }
            }
            List<GatewayFlowRuleEntity> flowRules = repository.saveAll(rules);
            return Result.ofSuccess(flowRules);
