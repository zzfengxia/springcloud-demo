 ...
 */
@RestController
@RequestMapping(value = "/gateway/api/nacos/")
public class GatewayApiNacosController {
    ...
    private DynamicRuleProvider<RuleEntityWrapper<ApiDefinitionEntity>> apiDefinitionProvider;
    @Autowired
    @Qualifier("apiDefinitionNacosPublisher")
    private DynamicRulePublisher<RuleEntityWrapper<ApiDefinitionEntity>> apiDefinitionPublisher;
    @Autowired
    ...
    public Result<List<ApiDefinitionEntity>> queryApis(String app) {
        ...
        try {
            RuleEntityWrapper<ApiDefinitionEntity> apiDefinitions = apiDefinitionProvider.getRules(app);
            if (apiDefinitions != null && !apiDefinitions.getRuleEntity().isEmpty()) {
                for (ApiDefinitionEntity entity : apiDefinitions.getRuleEntity()) {
                    entity.setApp(app);
                }
            }
            List<ApiDefinitionEntity> rules = repository.saveAll(apiDefinitions);
    ...
    public Result<ApiDefinitionEntity> addApi(HttpServletRequest request, @RequestBody AddApiReqVo reqVo) {
        ...
        // 先刷新缓存
        repository.refreshRuleCache(apiDefinitionProvider, entity.getApp());
    ...
    private void publishApis(String app) throws Exception {
        RuleEntityWrapper<ApiDefinitionEntity> rules = repository.findRuleByApp(app);
        apiDefinitionPublisher.publish(app, rules);
    }
}
