...
app.controller('GatewayApiNacosCtl', ['$scope', '$stateParams', 'GatewayApiNacosService', 'ngDialog', 'MachineService',
  function ($scope, $stateParams, GatewayApiNacosService, ngDialog, MachineService) {
    ...
    getApis();
    function getApis() {
      GatewayApiNacosService.queryApis($scope.app).success(
        function (data) {
    ...
    $scope.saveApi = function () {
      ...
      if (!GatewayApiNacosService.checkApiValid($scope.currentApi, apiNames)) {
        return;
      }
    ...
    function addNewApi(api) {
      GatewayApiNacosService.newApi(api).success(function (data) {
        if (data.code == 0) {
          // 延迟加载，数据发布到nacos需要一定时间
          setTimeout(getApis, 1000);
    ...
    function saveApi(api, edit) {
      GatewayApiNacosService.saveApi(api).success(function (data) {
        if (data.code == 0) {
          setTimeout(getApis, 1000);
          if (edit) {
            gatewayApiDialog.close();
    ...
    function deleteApi(api) {
      GatewayApiNacosService.deleteApi(api).success(function (data) {
        if (data.code == 0) {
          setTimeout(getApis, 1000);
          confirmDialog.close();
        } else {
